...
| RRT 기본 구조                        | 구현 완료 |
| RRT* parent selection / rewiring | 구현 완료 |
...
gantt
    ...
    RRT* parent selection / rewiring :done, d3, 2025-05-24, 14d
...
단순히 Nav2를 실행하거나 강화학습 policy를 적용한 것이 아니라, Safety Node, Wall Following, Follow-the-Gap, Pure Pursuit, Occupancy Grid, RRT/RRT*를 직접 구현하면서 자율주행 차량의 전체 파이프라인을 구성했습니다.
...
4. RRT* 기반 parent selection과 rewiring으로 경로 품질을 개선했다.
...
RRT* node가 벽 안쪽이나 장애물 영역에 생성되는 문제가 있었습니다.
...
RRT* node와 edge가 실제 주행 가능 영역에 생성되도록 개선되었습니다.
...
RRT* path는 생성되지만 차량이 실제로 따라가기 어려운 급격한 방향 전환이 포함되는 경우가 있었습니다.
